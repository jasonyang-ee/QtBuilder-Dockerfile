...
if [[ $BUILD == true ]]; then
    ...
    docker buildx build --target=artifact --output type=local,dest=$(pwd)/build-${VERSION}/ --platform linux/amd64,linux/arm64 --build-arg VERSION=${VERSION} --cache-from=type=local,src=$(pwd)/cache/ --cache-to=type=local,dest=$(pwd)/cache/,mode=max -f BuilderQt/Dockerfile .
fi

# Push Qt Builder Image
if [[ $PUSH == true ]]; then
    docker buildx build --target=building --push --platform linux/amd64,linux/arm64 -t jasonyangee/qt-builder:${VERSION} --build-arg VERSION=${VERSION} --cache-from=type=local,src=$(pwd)/cache/ --cache-to=type=local,dest=$(pwd)/cache/,mode=max -f BuilderQt/Dockerfile .
fi
